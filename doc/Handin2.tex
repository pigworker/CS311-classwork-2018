...
For each of the following questions, marks each of the claims 1 for True
...
{\bf Part A} Consider IMP-expression $e$ given by $\newe{x}{y + 2}{\doe{x : = x-1}{x + y}}$.
Which of the following claims are true?
\begin{itemize}
\item[1]. If $s$ is $,y := 4$ then $s | e \Downarrow  ,y:= 4, x:=5 | 9$
\item[2]. If $s$ is $,y:= 4 z:=5$ then $s | e \Downarrow  ,y:= 4, z:=5 | 9$
\item[3] If $s$ is $,y:= 4 x:=5$ then $s | e \Downarrow  ,y:= 4 | 9$
\item[4] If $s$ is $,z:=5$ then $s | e \Downarrow  ,z:=5 |$
\end{itemize}
